...
test_that("simEventCox simulates data in the right way", {
  ...
  # Fit some Cox models
  cox_cens <- survival::coxph(
    survival::Surv(tstart, tstop, Delta == 0) ~ L0 + A0 + L,
    data = data_obs
  )
  cox_death <- survival::coxph(
    survival::Surv(tstart, tstop, Delta == 1) ~ L0 + A0 + L,
    data = data_obs
  )
  cox_Disease <- survival::coxph(
    survival::Surv(tstart, tstop, Delta == 2) ~ L0 + A0,
    data = data_obs[L == 0]
  )
  ...
  # Fit new Cox models
  cox_cens2 <- survival::coxph(
    survival::Surv(tstart, tstop, Delta == 0) ~ L0 + A0 + L,
    data = new_data
  )
  cox_death2 <- survival::coxph(
    survival::Surv(tstart, tstop, Delta == 1) ~ L0 + A0 + L,
    data = new_data
  )
  cox_Disease2 <- survival::coxph(
    survival::Surv(tstart, tstop, Delta == 2) ~ L0 + A0,
    data = new_data[L == 0]
  )
  ...
  expect_true(all(
    coef_Disease >= ci_Disease[, 1] & coef_Disease <= ci_Disease[, 2]
  ))
})
