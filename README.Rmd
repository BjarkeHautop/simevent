...
One can perform interventions using the function `alphaSimDisease` and calculate
the effect of interventions using `intEffectAlphaDisease`.
...
and does therefore take many arguments. You can read about the different
arguments on the help page
...
function `intEffectAlphaTreat` can be used. This function simulates data from
the Drop In setting in two scenarios. Under the intervention on the shape
parameter \eqn{\eta} of the Drop In process, which is multiplied by
\code{alpha}, and a baseline (non-intervened) scenario. It computes the
proportion of individuals who experience death or Drop In by a specified time
\eqn{\tau} in the group \code{A0 = a0}, optionally returning years lost. The
function can also plot a sample of the event data for each scenario for
comparison.
